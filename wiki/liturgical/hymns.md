...
Browse `raw/liturgical/hymns/songs/index.md` (title → slide range → file), or search the per-song files directly:

```sh
grep -rl "ብፅዕት ነሽ ማርያም" raw/liturgical/hymns/songs/   # find the song file by a remembered line
```

Each song file is self-contained (bold-field frontmatter + full text) and records its `**Source slides:**` range back into `all-in-one.md` for provenance.
...
- **Tag hymns by season/theme** (Advent, Lent, Fasika, Marian, communion) — the profile counts above give a starting signal.
...
- `raw/liturgical/hymns/all-in-one.md` + `raw/liturgical/hymns/songs/` (666 files + `index.md`) — from `All in one 1.pptx`
